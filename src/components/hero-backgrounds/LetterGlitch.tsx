...
interface LetterGlitchProps {
  ...
  smooth?: boolean;
  characters?: string;
  /** Pause animation when not in viewport (default true) to reduce CPU usage. */
...
export default function LetterGlitch({
  ...
  const animate = () => {
    if (pauseWhenNotVisible && !isVisibleRef.current) {
    ...
    if (smooth) {
      ...
      if (frameCount.current % 2 === 0) handleSmoothTransitions();
    }
  ...
  useEffect(() => {
    if (!pauseWhenNotVisible || !containerRef.current) return;
  ...
  }, [glitchSpeed, smooth, pauseWhenNotVisible]);
